...
contract SupplyChain {
  ...
  */
  ...
  that the item with the given sku has the state ForSale. */
    modifier ForSale(uint _sku) {require(items[_sku].state == State.ForSale, "item state is not for sale"); _;}
    modifier sold(uint _sku) {require(items[_sku].state == State.Sold, "item state is not sold"); _;} 
    modifier shipped(uint _sku) {require(items[_sku].state == State.Shipped, "item state is not shipped"); _;}
    modifier received(uint _sku) {require(items[_sku].state == State.Received,"item state is not recieved"); _;}
